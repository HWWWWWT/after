...
if($act=="shenhe")
{
	$sql="update product set shenhe=1 where id=$id";
$result=mysql_query($sql);
if($result)
{
	echo "<script language=JavaScript>{window.alert('审核成功！');window.location.href='ProductCheck.php'}</script>";
}
else
{
exit ("失败了");
...
$sql = "SELECT * FROM product where title like '%$title%' and shenhe=0";
 $result=mysql_query($sql);
 $count = mysql_num_rows($result);
$size =10;
$pager = get_pager('',array(),$count,$page,$size);
$sql="select * from product where title like '%$title%' and shenhe=0 order by id DESC limit $pager[start],$pager[size]";
$result=mysql_query($sql);
$i=0;
...
echo "已审核";
	?>
            ...
            <td width="80" align="center" bgcolor="#ECF5FF">
			 <a href="?id=<?php echo $data[id]?>&act=shenhe">审核产品</a>
