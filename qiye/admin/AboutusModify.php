...
require("../mysql.php");
//读取修改数据
$sql="select * from qiyeinfo where id=$id";
...
$sql="update qiyeinfo set title='$title',content='$content' where id=$id";
$result=mysql_query($sql);
if($result)
{
	echo "<script language=JavaScript>{window.alert('修改成功！');window.location.href='Aboutus.php'}</script>";
}
else
{
exit ("失败了");
